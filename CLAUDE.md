...
Go library that provides struct-tag-driven configuration binding on top of [spf13/viper](https://github.com/spf13/viper) and [spf13/pflag](https://github.com/spf13/pflag). Define a config struct with `name`, `default`, `usage`, and `flag` tags, then call `Register` + `Load` to wire up pflags, env vars, and defaults automatically.
...
- **configs.go** — Public API. `Config` is a type alias for `*viper.Viper`. Two entry points:
  - `Register(v, fs, spec, prefix...)` — reflects over a struct to register pflag flags (with zero-value defaults), set viper defaults from `default` tags, and auto-bind env vars (`strings.ToUpper(key)`).
  - `Load(v, spec, prefix...)` — reads resolved viper values back into the struct fields. Call after `flag.Parse()` and `v.BindPFlags(fs)`.
- **utils.go** — Internal helpers:
  - `derefStructPtr` — shared validation for `Register`/`Load`: extracts prefix, validates spec is pointer-to-struct, returns dereferenced `reflect.Value`.
